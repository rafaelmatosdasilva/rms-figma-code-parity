...
A component's `strokeSides` (`'bottom'` | `'all'`) pins which CSS border sides may be drawn:
`'bottom'` requires `border-bottom` and forbids the `border:` shorthand; `'all'` requires the
shorthand. Crucially, `strokeSides` is **mandatory whenever Figma draws any stroke** —
