...
- `maxSnapshotAgeDays` *(optional, default off)* - a hard age ceiling for the snapshots, in days. Day-to-day, a plan without a REST refresh downgrades staleness to a non-failing advisory (so the audit still runs) - but that lets a snapshot drift indefinitely, which is how real DS drift (retuned tokens, a resized component) hides behind all-green gates. Set this (e.g. `14`) and Gate [1] **hard-fails** once any snapshot is older than the ceiling, **even when plan-limited** - because the Plugin API capture works on any plan, "we literally never refreshed" is a real problem, not a plan excuse. Below the ceiling, behaviour is unchanged.
- `versionLockStrict` *(optional, default `false`)* - promote Gate [1]'s whole-file `_figmaVersion` mismatch from an advisory to a **hard fail**. Off by default because Figma versions the whole file, so a bump is usually an unrelated edit elsewhere; turn it on for a DS file disciplined enough that any bump warrants a Phase 1 re-run.
