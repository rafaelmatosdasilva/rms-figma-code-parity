...
Config: `styleguide: { template, out, iconSource, plugins? }`. `plugins` is `[{ key, match }]`, a short usage label per app and a fragment of its source path; without it each app in `paths.plugins` is labelled by its own name. Because it renders **only** what the
