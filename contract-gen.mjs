...
function buildLlms(built, tokens, tokenCount, catalog = null) {
  ...
  if (catalog && Object.keys(catalog.components).length) {
    lines.push('## Catalog (for UI generators)', '',
      '- [catalog.json](./catalog.json) — every component with its props, allowed values and allowed children. Check generated UI with `rms-figma-code-parity --check-ui <file>`.', '',
      ...catalog.rules.map((r, i) => `${i + 1}. ${r}`), '', catalogTable(catalog), '');
  }
  return lines.join('\n');
}
...
export async function generateContracts(ROOT, cfg, opts = {}) {
  ...
  // The component catalog for UI generators (ui-catalog.mjs): from the contracts, plus the code
  // snapshot's prop names and nesting when it still matches the code.
  let catalog = null;
  try {
    let code = null, selectorFor = null;
    try { code = await (await import('./code-capture.mjs')).readFreshSnapshot(ROOT, cfg); } catch { /* optional */ }
    try { selectorFor = (await loadLocator(ROOT, cfg)).selectorFor; } catch { /* optional */ }
    catalog = buildCatalog(built, { code, selectorFor });
